...
RulePkt* RulePkt::copy() {
  ...
  copy->setCompositeEventTemplate(new CompositeEventTemplate(this->getCompositeEventTemplate()->getEventType()));
  return copy;
}
...
bool RulePkt::addAggregate(int eventType, Constraint constraints[],
  ...
  // This causes a crash with a valid aggregation function
  /*if (lowerId >= 0 && lowerId <= upperId) {
    return false;
  }*/
  Aggregate a;
