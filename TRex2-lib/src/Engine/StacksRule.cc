...
StacksRule::~StacksRule() {
  ...
  for (auto it : negations)
    delete it.second;
  //delete eventGenerator;  // espen: This causes segmentation fault.
}
...
void StacksRule::createComplexEvents(list<PartialEvent*>* partialEvents,
  ...
  for (auto pe : *partialEvents) {
    ...
    PubPkt *lastPacket = pe->indexes[0];
    // We record the thread Id and pkt Id (running)
    traceEvent(6, {(int)syscall(SYS_gettid), lastPacket->pkt_id});
    if (++complexEventCreated % 100000 == 0) {
      std::cout << "Created complex event " << complexEventCreated << std::endl;
    }
