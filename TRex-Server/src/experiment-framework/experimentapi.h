...
public:
    ...
    std::string StartRuntimeEnv();
    std::string StopRuntimeEnv();
    std::string RetEndOfStream(int milliseconds);
    std::string HandleEvent(YAML::Node yaml_event);
    std::string AddTpIds(std::vector<int> tracepointIds);
    std::string EndExperiment();
